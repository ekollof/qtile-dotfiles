...
# Set wallpaper
if [ -f ~/.wallpaper ]; then
    log "Setting wallpaper from ~/.wallpaper"
    if [ -x ~/bin/wallpaper.ksh ]; then
        nohup ~/bin/wallpaper.ksh -R >/dev/null 2>&1 &
    else
        # Fallback to feh
        if command -v feh >/dev/null 2>&1; then
            wallpaper=$(realpath ~/.wallpaper 2>/dev/null)
            if [ -n "$wallpaper" ] && [ -f "$wallpaper" ]; then
                nohup feh --bg-max "$wallpaper" >/dev/null 2>&1 &
            fi
        fi
    fi
else
    log "No wallpaper file found at ~/.wallpaper"
fi
