#!/usr/bin/env python3
"""
Popup Notification System for Qtile

@brief Native X11 popup notifications using xcffib
@author Andrath

This module provides popup notification windows using xcffib for native X11
window creation with precise positioning and styling.

Features:
- Native X11 windows with precise positioning
- Automatic stacking in screen corners
...
@note Falls back to simple notify-send if xcffib unavailable
"""
...
class PopupNotification:
    """Data structure for popup notification"""
    title: str
    message: str
    urgency: str = "normal"
    timeout: float = 5.0
...
class PopupManager:
    """
    Popup notification manager using native X11 windows

    Creates popup notification windows using xcffib for precise positioning
    and styling. Falls back to notify-send if xcffib is unavailable.
    """
    ...
    def __init__(self, color_manager):
        ...
        # Initialize XCB if available
        if XCB_AVAILABLE:
            self._init_xcb()
        else:
            logger.warning("xcffib not available - using notify-send fallback")
    ...
    def _init_xcb(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to initialize XCB: {e}")
            self._xcb_connection = None
            return False
    ...
    def configure(self, config: Dict[str, Any]) -> None:
        ...
        logger.info(f"Popup manager configured: {self.config['position']}")

    def show_notification(self, title: str, message: str, urgency: str = "normal", timeout: int = 5000) -> None:
        """Show popup notification"""
        if XCB_AVAILABLE and self._xcb_connection:
            self._show_xcb_notification(title, message, urgency, timeout)
        else:
            self._show_fallback_notification(title, message, urgency, timeout)

    def _show_xcb_notification(self, title: str, message: str, urgency: str, timeout: int) -> None:
        """Show notification using XCB"""
        if not qtile:
            logger.warning("Qtile not available")
            return

        # Convert timeout
        timeout_seconds = timeout / 1000.0 if timeout > 0 else 0.0

        # Apply urgency timeouts
        if urgency == "critical":
    ...
    def _show_fallback_notification(self, title: str, message: str, urgency: str, timeout: int) -> None:
        """Show notification using notify-send fallback"""
        try:
            cmd = [
                "notify-send",
                "-t", str(timeout),
            ...
            logger.debug(f"Sent fallback notification: {title}")
        except Exception as e:
            logger.warning(f"Fallback notification failed: {e}")

    def _create_xcb_window(self, notification: PopupNotification) -> Optional[int]:
        ...
        try:
            ...
            value_list = [
                bg_pixel,
                (xcffib.xproto.EventMask.Exposure | xcffib.xproto.EventMask.ButtonPress),
                1,  # Override redirect
            ]
            ...
            self._xcb_connection.core.CreateWindow(
                ...
                self.config.get("border_width", 2),
                xcffib.xproto.WindowClass.InputOutput,
                self._screen.root_visual,
    ...
    def _set_window_properties(self, window_id: int, notification: PopupNotification) -> None:
        """Set window properties"""
        try:
            # Set window title
            ...
            self._xcb_connection.core.ChangeProperty(
                ...
                xcffib.xproto.Atom.STRING,
                8, len(wm_class), wm_class
            )
        except Exception as e:
            logger.debug(f"Error setting window properties: {e}")

    def _draw_notification(self, notification: PopupNotification) -> None:
    ...
    def _draw_background(self, ctx: cairocffi.Context, notification: PopupNotification) -> None:
        """Draw background"""
        colors = self.color_manager.get_colors()
        bg_color = colors.get("special", {}).get("background", "#000000")
        ...
        self._draw_rounded_rectangle(ctx, 0, 0, notification.width, notification.height,
                                   self.config.get("corner_radius", 8))
        ctx.fill()

    def _draw_border(self, ctx: cairocffi.Context, notification: PopupNotification) -> None:
        """Draw border"""
        colors = self.color_manager.get_colors()

        # Choose border color by urgency
        ...
        self._draw_rounded_rectangle(ctx, border_width/2, border_width/2,
                                   notification.width - border_width,
                                   notification.height - border_width,
                                   self.config.get("corner_radius", 8))
        ctx.stroke()

    def _draw_text(self, ctx: cairocffi.Context, notification: PopupNotification) -> None:
        """Draw text content"""
        colors = self.color_manager.get_colors()

        # Choose text color by urgency
        if notification.urgency == "critical":
        ...
        # Set font
        ctx.select_font_face(self.config.get("font_family", "DejaVu Sans"),
                           cairocffi.FONT_SLANT_NORMAL, cairocffi.FONT_WEIGHT_NORMAL)
        ...
        if notification.message:
            ...
            # Word wrap message
            message_lines = self._wrap_text(ctx, notification.message,
    ...
    def _calculate_position(self, stack_index: int) -> Tuple[int, int]:
        ...
        if not qtile or not qtile.current_screen:
            return (100, 100)
    ...
    def _dismiss_oldest(self) -> None:
        ...
        if self.active_notifications:
            ...
            logger.debug(f"Dismissed oldest: {oldest.title}")

    def _destroy_window(self, notification: PopupNotification) -> None:
        """Destroy XCB window"""
    ...
    def dismiss_all(self) -> None:
        ...
        self._cleanup_scheduled = False
        logger.info(f"Dismissed {count} notifications")
...
def create_popup_manager(color_manager) -> PopupManager:
    """Create popup notification manager"""
    try:
        manager = PopupManager(color_manager)
        logger.info("Popup notification manager created")
        return manager
    except Exception as e:
        logger.error(f"Failed to create popup manager: {e}")
        return PopupManager(color_manager)


def show_popup(title: str, message: str, urgency: str = "normal",
               timeout: int = 5000, manager=None) -> None:
    """Show popup notification"""
    if manager:
        manager.show_notification(title, message, urgency, timeout)
    else:
        logger.warning("No popup manager provided")
