...
class TestSVGBuilder:
    ...
    def test_build_with_defs(self):
        ...
        builder: SVGBuilder = SVGBuilder(24, 24)
        builder.defs.append(
            '<linearGradient id="grad1"><stop offset="0%" stop-color="#ff0000"/><stop offset="100%" stop-color="#0000ff"/></linearGradient>'
        )
...
class TestSVGManipulator:
    ...
    def test_recolor_svg_stop_color(self):
        ...
        manipulator: SVGManipulator = SVGManipulator()
        original_svg: SVGIcon = SVGIcon(
            content='<svg><stop stop-color="#ff0000"/></svg>', width=24, height=24
        )
    ...
    def test_save_svg_success(
        self, mock_mkdir: MagicMock, mock_parent: MagicMock, mock_write_text: MagicMock
    ):
...
class TestIconGenerator:
    ...
    def test_battery_icon_charging(self):
        ...
        assert "<svg" in svg
        assert (
            'fill="' + generator.colors["warning"] + '"' in svg
        )  # Charging uses warning color
...
class TestGlobalFunctions:
    ...
    def test_create_themed_icon_cache(
        self, mock_write_text: MagicMock, mock_mkdir: MagicMock
    ):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result: dict[str, str] = create_themed_icon_cache(
                mock_color_manager, icon_dir, 24
            )
